...
df0<-read.table('pas_segm_done.csv',sep=' ',header=F)
...
for (pmx in 1:mx){
	park = ecox_list[pmx]
	print(park)
	name <- paste(park,'_movar_results.csv',sep='')
	df0<-read.table(name,sep=' ',header=F)
	df1<-df0[!is.nan(df0$V2),]

	namem <- paste(park,'_moran_mean.csv',sep='')
	df0m<-read.table(namem,sep=' ',header=F)
	namev <- paste(park,'_var_mean.csv',sep='')
	df0v<-read.table(namev,sep=' ',header=F)
	#df1<-df0[!is.nan(df0$V2),]

	name2 <- paste(park,'_movar_thresholds.csv',sep='')
	...
	png(paste(park,'_moran_var_mean.png',sep=''))
	...
	res<-round(x.points[length(x.points)])
	...
	res2[1,1]<-park
	write.table(res0,paste(park,'_optim_thresholds.csv',sep=''),sep=' ',col.names=F,row.names=F)
	write.table(res2,'overall_optim_thresholds.csv',append=T,sep=' ',col.names=F,row.names=F)



	namef <- paste('park_',park,'_hri_results',res,'.csv',sep='')
	...
	png(paste('hclust_',park,'_',res,'_segms_mean.png',sep=''))
	plot(hclust_mh,hang=-1,main=park);try(rect.hclust(hclust_mh,h=mean(hclust_mh$height)))
	dev.off()

	png(paste('NMDS_',park,'_',res,'_segms_mean.png',sep=''))
	plot(mds_mh)
...
#	rownames(hrin)<-hri[,12]
	...
	rpn=paste('radarplot_',park,'_',res,'_segms_mean.png',sep='')
	ggsave(filename=rpn)
	dev.off()

	segm_pa<-readOGR(dsn='.',lay=paste('park_segm_',park,'_',res,'_diss',sep=''))

	merge(segm_pa,hrin[,c(2,12)],by='segm_id')->segm_pa_class

	writeOGR(segm_pa_class,dsn='.',paste('park_segm_',park,'_',res,'_class',sep=''),driver="ESRI Shapefile")

	rpn2=paste('map_',park,'_',res,'_segms_hclust.png',sep='')
	png(paste(rpn2))
