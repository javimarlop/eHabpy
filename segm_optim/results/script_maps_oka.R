...
# library(gridExtra)
 ...
 pa0<-read_sf('park_segm_555577555_1_class.shp')#'park_segm_2572_7_class.shp')
...
# set_defaults(map_service = "esri", map_type = "world_physical_map")
 ...
 hft1_lp<-raster('../../results/30907_555577555_1_lp.tif')
 hft2_lp<-raster('../../results/30907_555577555_2_lp.tif')
 hft3_lp<-raster('../../results/30907_555577555_3_lp.tif')
 hft4_lp<-raster('../../results/30907_555577555_4_lp.tif')
 hft5_lp<-raster('../../results/30907_555577555_5_lp.tif')
 hft6_lp<-raster('../../results/30907_555577555_6_lp.tif')  
 #base<-st_bbox(hft1)
...
mhfts0<-mosaic(hft1,hft2,hft3,hft4,hft5,hft6,fun=sum) ####### COMBINE EXTENTS ######################################
...
# HFTs
hft<-ggplot(data = pa) + annotation_map_tile('stamenwatercolor', zoom=9) + geom_sf(aes(fill = as.factor(hclst_m))) + annotation_scale(location = "tl") + scale_fill_brewer(palette = 'Set1') + ggtitle(label = "Okavango", subtitle = "HFTs") + annotation_north_arrow(location = "tr", which_north = "true") + guides(fill=guide_legend("HFT"))

ggsave('hft_oka.png',hft)
...
phfts<-ggplot(data = pa) + annotation_map_tile('stamenwatercolor', zoom=9) + geom_sf(fill='transparent') + annotation_scale() + ggtitle(label = "Okavango", subtitle = "Similar landscape patches to all HFTs") + annotation_north_arrow(location = "br", which_north = "true") + layer_spatial(trim(mhfts)) + scale_fill_continuous(na.value = "transparent") + theme(legend.position = "none") # + guides(fill=guide_legend("HFT"))

ggsave('hfts_oka.png',phfts) #.tiff

plot_hfts<-function(name=''){
hfts<-ggplot(data = pa) + annotation_map_tile('stamenwatercolor', zoom=9) + geom_sf(fill='transparent') + annotation_scale() + ggtitle(label = "Okavango", subtitle = "Similar landscape patches") + annotation_north_arrow(location = "br", which_north = "true") + layer_spatial(trim(get(name))) + scale_fill_continuous(na.value = "transparent") + theme(legend.position = "none") # + guides(fill=guide_legend("HFT"))
ggsave(paste('oka_',name,'_simil.png',sep=''),hfts) #.tiff
return(hfts)
}
...
#png('oka_simil.png',width=1680,height=1050) 
