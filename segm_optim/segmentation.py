...
#  jj = 1
#  it = [0.01, 0.05, 0.1, 0.2]
#  for thri in it:
#	  jj= jj + 1
#  	  pa2i = pa+'v2i_'+str(jj)
#  	  pa2si = pa+'v2i_'+str(jj-1)
#  	  grass.run_command('g.region',vect=pa0,res=1000)
#	  if thri==0.01:
#	  	grass.run_command('i.segment', group='segm2', output=pa2i, threshold=thri, method='region_growing', similarity='euclidean', memory='100000', iterations='20',overwrite=True) # minsize=minarea
#	  else:
#	  	grass.run_command('i.segment', group='segm2', output=pa2i, threshold=thri, method='region_growing', similarity='euclidean', memory='100000', iterations='20',seed=pa2si,overwrite=True) # minsize=minarea
  ...
  for thr in reps:
	  ...
	  j= j + 1
	  if thr==0.1:
	  	grass.run_command('i.segment', group='segm2', output=pa2, threshold=thr, method='region_growing', minsize=minarea, similarity='euclidean', memory='100000', iterations='20',overwrite=True) #  ,seed=pa2i minsize=minarea,
	  else:
	  	grass.run_command('i.segment', group='segm2', output=pa2, threshold=thr, method='region_growing', similarity='euclidean', memory='100000', iterations='20',seed=pa2s,overwrite=True) # minsize=minarea
	  #grass. message ("cropping the segments")
