...
def	ehabitat(ecor,nw,nwpathout):
	...
	if os.path.isfile(csvname) == False:
		wb = open(csvname,'a')
		wb.write('ecoregion wdpaid averpasim hr2aver pxpa hriaver nfeatsaver lpminsize lpmaxsize aggregation treepamin treepamax eprpamin eprpamax prepamin prepamax biopamin biopamax slopepamin slopepamax ndwipamin ndwipamax ndvimaxpamin ndvimaxpamax ndviminpamin ndviminpamax hpamin hpamax')
		wb.write('\n')
		wb.close()
				...
				if xoff>0 and yoff>0 and go == 1:
					...
					if ind_pa.shape[0]>4 and ind_pa.shape[1]>1:
						...
						if xoff>0 and yoff>0 and ratiogeom < 100: #	also	check	if results	are	not	empty?
							...
							psizes = nd.labeled_comprehension(hr1averr, num_featuresaver, lbls, np.count_nonzero, int, 0)
							pszmin = psizes.min()
							pszmax = psizes.max()
