...
#### Modernized for Python 3 / GRASS GIS 8 / GDAL 3+ (2024)
#### License: CC BY-SA 3.0
#### Control files: ecoregs_done.csv
#### Inputs variables: variables mentioned in the script located in the inVars folder (Mollweide projection); 'pas/ecoregs.csv'; individual ecoreg csv files; individual HFT raster files
...
# GDAL >= 3.7 warns (and GDAL >= 4.0 will error) unless exceptions are enabled explicitly.
gdal.UseExceptions()
ogr.UseExceptions()

# Statistic used to aggregate the per-HFT similarity value (AveHFTSim / MedianHFTSim).
# ehab_optim_median.py: uses np.median for the per-HFT similarity aggregation
# (MedianHFTSim). The per-variable *pamean columns still use np.mean in BOTH
# variants (this matches the original code). Generated from ehab_optim.py.
AGG = np.median
SIM_LABEL = 'MedianHFTSim'


def merge_rasters(inputs, output, output_type='Int32'):
	"""Replacement for the old ``os.system('gdal_merge.py ...')`` call.

	Uses the packaged osgeo_utils.gdal_merge module (identical behaviour to the
	CLI tool, but no dependency on it being on PATH). Later inputs overwrite
	earlier ones on overlap, exactly like gdal_merge.py.
	"""
	argv = ['', '-o', output, '-ot', output_type] + [str(i) for i in inputs]
	try:
		from osgeo_utils import gdal_merge
		gdal_merge.main(argv)
	except Exception:
		# Fallback for environments where gdal_merge.py is still on PATH.
		os.system('gdal_merge.py ' + ' '.join(str(i) for i in inputs) +
				  ' -o ' + str(output) + ' -ot ' + output_type)
...
def	ehabitat(ecor, nw, nwpathout):
	...
	if nwpathout == '':
		...
		outdir = os.path.join(nwpathout, 'results')
		safelyMakeDir(outdir)
	...
	#	LOCAL FOLDER
	csvname1 = os.path.join(outdir, 'ecoregs_done.csv')
	print(csvname1)
	...
	ecofile = os.path.join(nwpath, 'ecoregs', ef)
	print(ecofile)
	avail = os.path.isfile(ecofile)
	if avail == True:
		eco_csv = str(ecor) + '.csv'
		print(eco_csv)
		ecoparksf = os.path.join(nwpath, 'pas', eco_csv)
		print(ecoparksf)
		...
		print(ecoparksf)
		pa_list0 = np.genfromtxt(ecoparksf, dtype=str)	# crear este archivo en subpas!
		...
		for	px in range(0, n):  # 0,n
			...
			pa = pa_list[px]
			print(pa)
			txtlist = str(pa).split('_')
			outfile = os.path.join(outdir, str(ecor) + '_' + str(pa) + '.tif')
			outfile2 = os.path.join(outdir, str(ecor) + '_' + str(pa) + '_lp.tif')
			outfile3 = os.path.join(outdir, str(ecor) + '_' + str(pa) + '_mask.tif')
			pa_infile = 'pa_' + str(pa) + '.tif'

			pa4 = os.path.join(nwpath, 'pas', pa_infile)
			print(pa4)
			...
			if done == False and avail2 == True:
				...
				if xoff > 0 and yoff > 0 and go == 1:
					...
					if ind_pa.shape[0] > 4 and ind_pa.shape[1] > 1:
						...
						# Modernization: Python 2 used ``np.where(pmh<=0.001, None, pmh)`` which
						# produced an object array; None sorted below floats so ``.max()`` and
						# ``>0`` still worked. In Python 3 that raises TypeError. Using 0.0 is
						# numerically identical downstream (all uses filter on ``>0``).
						pmhh = np.where(pmh <= 0.001, 0.0, pmh)
						...
						if xoff > 0 and yoff > 0 and pmhhmax > 0.01 and hr1sum > 1 and maxmh != float('NaN'):  # and ratiogeom < 100: #	also	checks	if results	are	not	empty
							...
							merge_rasters([ecofile, pa4], outfile3, output_type='Int32')
							...
							if hri_pa_bb02.shape == hri_pa_bb03.shape:
								...
								hr1averpa = round(AGG(hri_pa0[~np.isnan(hri_pa0)]), 2)
					...
					wb = open(csvname, 'a')
					var = str(ecor) + ' ' + txtlist[0] + ' ' + str(pa) + ' ' + str(hr1averpa) + ' ' + str(hr2aver) + ' ' + str(pxpa) + ' ' + str(hr1insumaver) + ' ' + str(hr3aver) + ' ' + str(num_featuresaver) + ' ' + str(lpratio) + ' ' + str(lpratio2) + ' ' + str(numpszok) + ' ' + str(pszmax) + ' ' + str(aggregation) + ' ' + str(treepamin) + ' ' + str(treepamax) + ' ' + str(eprpamin) + ' ' + str(eprpamax) + ' ' + str(prepamin) + ' ' + str(prepamax) + ' ' + str(biopamin) + ' ' + str(biopamax) + ' ' + str(slopepamin) + ' ' + str(slopepamax) + ' ' + str(ndwipamin) + ' ' + str(ndwipamax) + ' ' + str(ndvimaxpamin) + ' ' + str(ndvimaxpamax) + ' ' + str(ndviminpamin) + ' ' + str(ndviminpamax) + ' ' + str(hpamin) + ' ' + str(hpamax) + ' ' + str(treepamean) + ' ' + str(eprpamean) + ' ' + str(prepamean) + ' ' + str(biopamean) + ' ' + str(slopepamean) + ' ' + str(ndwipamean) + ' ' + str(ndvimaxpamean) + ' ' + str(ndviminpamean) + ' ' + str(hpamean)
					wb.write(var)
					wb.write('\n')
					wb.close()
...
def	run_batch():
	eco_list0 = np.genfromtxt(os.path.join(nwpath, 'pas', 'ecoregs.csv'), dtype=int)	#	crear	este	archivo	en	subpas!
